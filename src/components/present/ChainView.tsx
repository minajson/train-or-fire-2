    ...
    >
      ...
      {showBar ? (
        <motion.div
          ...
          style={{ originX: 0 }}
          className={cn(
            "absolute inset-y-[0.4vh] left-0 right-0 rounded-r-lg",
      ...
      <div
        className={cn(
          ...
          "h-[4.4vh] w-[4.4vh] min-h-8 min-w-8 rounded-md text-[max(0.8rem,1.6vh)]",
          link.terminal
            ? "bg-fire text-white"
            : winner
      ...
      <div className="relative ml-[1.4vw] min-w-0 flex-1">
        <div
          className={cn(
      ...
      {showBar ? (
        ...
        <div className="relative ml-[1.5vw] shrink-0">
          <span className="inline-block rounded-full bg-signal-wash px-[1.1vw] py-[0.8vh] text-stage-sm font-semibold uppercase tracking-[0.08em] text-signal">
            {link.opportunity}
          </span>
        </div>
      ) : null}
...
export function ChainView({
  ...
  return (
    <ol className={cn("relative flex min-h-0 flex-1 flex-col gap-[0.8vh]", className)}>
      {/* The spine. Centred on the number chips, inset so it does not overrun. */}
      <div
        aria-hidden="true"
        className="absolute bottom-[4vh] left-[2.2vh] top-[4vh] w-px bg-rule"
      />
      {CHAIN.map((link, i) => (
        <ChainRow
          key={link.n}
